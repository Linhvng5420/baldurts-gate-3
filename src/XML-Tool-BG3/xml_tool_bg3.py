...
def _keyword_matches(text, keyword, match_case=False, match_whole_word=False):
    """
    Kiểm tra xem từ khóa có khớp với text dựa trên các tùy chọn tìm kiếm
    
    Args:
        text (str): Text cần kiểm tra
        keyword (str): Từ khóa tìm kiếm
        match_case (bool): Có phân biệt hoa thường không
        match_whole_word (bool): Có tìm kiếm từ hoàn chỉnh không
...
def _count_words_in_content(xml_element):
    """
    Đếm số từ trong phần text content của XML element
    
    Args:
        xml_element (str): Phần tử XML content
        
    Returns:
        int: Số từ trong content text
    """
    # Trích xuất phần text content (giữa > và </content>)
    text_match = re.search(r'>([^<]*)</content>', xml_element)
    if not text_match:
        return 0
    ...
    # Đếm số từ bằng cách tách theo khoảng trắng
    words = content_text.split()
    return len(words)

# Define functions
def tim_kiem_noi_dung(file_path, search_pattern, match_case=False, match_whole_word=False, search_mode="full", limit_words=False, max_words=2):
    """
    Tìm kiếm nội dung trong file XML với các tùy chọn khác nhau
    
    Args:
        file_path (str): Đường dẫn đến file XML
        search_pattern (str): Từ khóa tìm kiếm (phân cách bằng dấu phẩy)
        match_case (bool): Có phân biệt hoa thường không
        match_whole_word (bool): Có tìm kiếm từ hoàn chỉnh không
        search_mode (str): Chế độ tìm kiếm - "content_only", "full"
        limit_words (bool): Có giới hạn số từ trong kết quả không
        max_words (int): Số từ tối đa trong kết quả (nếu limit_words=True)
        
    Returns:
        list: Danh sách các phần tử content tìm thấy
    """
    try:
...
class SearchToolUI:
    ...
    def __init__(self, master):
        ...
        self.tab_control.add(self.tab5, text='Ghép file XML')
    ...
    def setup_search_tab(self):
        ...
        # Frame cho các tùy chọn tìm kiếm
        options_frame = ttk.Frame(frame)
        options_frame.pack(fill="x", padx=5, pady=5)
        
        ttk.Label(options_frame, text="Tùy chọn tìm kiếm:").pack(side=tk.LEFT)
        
        # Checkbox cho Match Case
        self.match_case_var = tk.BooleanVar()
        match_case_check = ttk.Checkbutton(options_frame, text="Match Case (Phân biệt hoa thường)", 
                                         variable=self.match_case_var)
        match_case_check.pack(side=tk.LEFT, padx=10)
        
        # Checkbox cho Match Whole Word
        self.match_whole_word_var = tk.BooleanVar()
        match_whole_word_check = ttk.Checkbutton(options_frame, text="Match Whole Word (Từ hoàn chỉnh)", 
                                               variable=self.match_whole_word_var)
        match_whole_word_check.pack(side=tk.LEFT, padx=10)
        
        # Frame cho tùy chọn giới hạn số từ
        word_limit_frame = ttk.Frame(frame)
        word_limit_frame.pack(fill="x", padx=5, pady=5)
        
        # Checkbox cho giới hạn số từ
        self.limit_words_var = tk.BooleanVar()
        limit_words_check = ttk.Checkbutton(word_limit_frame, text="Giới hạn số từ trong dòng tìm được:", 
                                          variable=self.limit_words_var,
                                          command=self.toggle_word_limit)
        limit_words_check.pack(side=tk.LEFT)
        
        # Entry cho số từ tối đa
        ttk.Label(word_limit_frame, text="Số từ tối đa:").pack(side=tk.LEFT, padx=(10, 5))
        self.max_words_var = tk.StringVar(value="2")
        self.max_words_entry = ttk.Entry(word_limit_frame, textvariable=self.max_words_var, width=5)
        ...
        self.search_mode_var = tk.StringVar(value="full")  # Mặc định là tìm cả code
        
        content_only_radio = ttk.Radiobutton(search_mode_frame, 
                                           text="🔍 Chỉ tìm Nội Dung (text bên trong thẻ content)", 
                                           variable=self.search_mode_var, 
                                           value="content_only")
        content_only_radio.pack(side=tk.LEFT, padx=10, pady=5)
        
        full_search_radio = ttk.Radiobutton(search_mode_frame, 
                                          text="🔍 Tìm cả Code (bao gồm thẻ XML, thuộc tính và nội dung)", 
                                          variable=self.search_mode_var, 
                                          value="full")
        full_search_radio.pack(side=tk.LEFT, padx=10, pady=5)
        
        # Thông báo hướng dẫn cho chế độ tìm kiếm
    ...
    def setup_merger_tab(self):
        """Thiết lập tab ghép file XML"""
        frame = ttk.LabelFrame(self.tab5, text="Ghép nhiều file XML thành một file")
        frame.pack(fill="both", expand=True, padx=10, pady=10)
        ...
        info_text = "💡 Ghép nhiều file XML thành một file duy nhất"
        ttk.Label(info_frame, text=info_text, font=("Arial", 9), foreground="blue").pack(side=tk.LEFT)
        
        # Danh sách các file XML sẽ ghép
        self.merger_xml_files = []
        
        # Frame nhập đường dẫn
        input_frame = ttk.LabelFrame(frame, text="Thêm File XML", padding="5")
        ...
        self.merger_path_entry.pack(side=tk.LEFT, padx=5, fill="x", expand=True)
        
        # Nút Browse
        ttk.Button(path_row, text="Duyệt...", command=self.merger_browse_file).pack(side=tk.LEFT, padx=2)
        
        # Nút lấy từ clipboard
        ttk.Button(path_row, text="Clipboard", command=lambda: self.parse_clipboard(self.merger_path_var)).pack(side=tk.LEFT, padx=2)
        ...
        # Nút thêm file
        ttk.Button(button_row, text="➕ Thêm File", command=self.merger_add_file).pack(side=tk.LEFT, padx=2)
        ...
        self.merger_file_listbox = tk.Listbox(listbox_frame, height=8)
        scrollbar = ttk.Scrollbar(listbox_frame, orient="vertical", command=self.merger_file_listbox.yview)
        self.merger_file_listbox.configure(yscrollcommand=scrollbar.set)
        
        self.merger_file_listbox.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")
        
        # Nút xóa file được chọn
        ttk.Button(list_frame, text="❌ Xóa Đã Chọn", command=self.merger_remove_selected).pack(pady=5)
        
        # Frame tên file output
        output_frame = ttk.LabelFrame(frame, text="File Kết Quả", padding="5")
        output_frame.pack(fill="x", padx=5, pady=5)
        
        output_row = ttk.Frame(output_frame)
        output_row.pack(fill="x")
        
        ttk.Label(output_row, text="Tên file:").pack(side=tk.LEFT)
        self.merger_output_var = tk.StringVar(value="merged_output.xml")
        ttk.Entry(output_row, textvariable=self.merger_output_var, width=30).pack(side=tk.LEFT, padx=5)
        
        # Nút ghép file
        ttk.Button(output_row, text="🔗 Ghép File", command=self.merger_merge_files).pack(side=tk.LEFT, padx=10)
    ...
    def toggle_word_limit(self):
        """Bật/tắt ô nhập số từ tối đa"""
        if self.limit_words_var.get():
            self.max_words_entry.config(state="normal")
        else:
            self.max_words_entry.config(state="disabled")
    ...
    def run_search(self):
        ...
        match_whole_word = self.match_whole_word_var.get()
        search_mode = self.search_mode_var.get()
        limit_words = self.limit_words_var.get()
        
        # Lấy giá trị max_words và kiểm tra tính hợp lệ
        max_words = 2  # Giá trị mặc định
        if limit_words:
            try:
                max_words = int(self.max_words_var.get())
                if max_words <= 0:
                    messagebox.showerror("Lỗi", "Số từ tối đa phải là số nguyên dương")
                    return
            except ValueError:
                messagebox.showerror("Lỗi", "Số từ tối đa phải là số nguyên hợp lệ")
                return
        ...
        self.clear_output()
        threading.Thread(target=self._search_thread, args=(file_path, search_content, match_case, match_whole_word, search_mode, limit_words, max_words)).start()

    def _search_thread(self, file_path, search_content, match_case=False, match_whole_word=False, search_mode="full", limit_words=False, max_words=2):
        """
        ...
        Args:
            ...
            match_whole_word (bool): Có tìm kiếm từ hoàn chỉnh không
            search_mode (str): Chế độ tìm kiếm - "content_only", "full"
            limit_words (bool): Có giới hạn số từ không
        ...
        try:
            ...
            # Thực hiện tìm kiếm với các tùy chọn mới
            self.search_results = tim_kiem_noi_dung(file_path, search_content, match_case, match_whole_word, search_mode, limit_words, max_words)
    ...
    # === Merger Functions ===
    def merger_browse_file(self):
        """Mở dialog chọn file XML"""
        filepath = filedialog.askopenfilename(
            title="Chọn file XML",
            filetypes=[("XML files", "*.xml"), ("All files", "*.*")]
        )
        if filepath:
            self.merger_path_var.set(filepath)
            # Thêm vào lịch sử
            self.add_to_history(filepath)
            
    def merger_add_file(self):
        """Thêm file vào danh sách merger"""
        file_path = self.merger_path_var.get().strip()
        
        if not file_path:
            self.print_to_output("❌ Vui lòng nhập đường dẫn file")
            return
            
        # Kiểm tra file tồn tại
        if not os.path.exists(file_path):
            self.print_to_output(f"❌ File không tồn tại: {file_path}")
            return
            
        # Kiểm tra extension
        if not file_path.lower().endswith('.xml'):
            self.print_to_output("❌ File phải có extension .xml")
            return
            
        # Kiểm tra duplicate
        if file_path in self.merger_xml_files:
            self.print_to_output("⚠️ File đã có trong danh sách")
            return
            
        # Thêm vào danh sách
        self.merger_xml_files.append(file_path)
        self.merger_file_listbox.insert(tk.END, os.path.basename(file_path))
        
        # Xóa nội dung entry và thêm vào lịch sử
        self.merger_path_var.set("")
        self.add_to_history(file_path)
        
        self.print_to_output(f"✅ Đã thêm file: {os.path.basename(file_path)}")
        
    def merger_add_multiple_files(self):
        """Thêm nhiều file cùng lúc"""
        filenames = filedialog.askopenfilenames(
            title="Chọn nhiều file XML",
            filetypes=[("XML files", "*.xml"), ("All files", "*.*")]
        )
        ...
        if filenames:
            ...
            for filename in filenames:
                # Kiểm tra duplicate
                if filename in self.merger_xml_files:
                    skipped_files.append(f"{os.path.basename(filename)} - Đã có trong danh sách")
                    continue
                    
                # Thêm vào danh sách
                self.merger_xml_files.append(filename)
                self.merger_file_listbox.insert(tk.END, os.path.basename(filename))
                self.add_to_history(filename)
                added_count += 1
            
            # Hiển thị kết quả
            self.print_to_output(f"✅ Đã thêm {added_count} file thành công!")
            if skipped_files:
                self.print_to_output(f"⚠️ Bỏ qua {len(skipped_files)} file:")
                for skip_info in skipped_files:
                    self.print_to_output(f"   • {skip_info}")
                    
    def merger_remove_selected(self):
        """Xóa file được chọn trong listbox"""
        selection = self.merger_file_listbox.curselection()
        if not selection:
            self.print_to_output("⚠️ Vui lòng chọn file để xóa")
            return
            
        index = selection[0]
        file_path = self.merger_xml_files[index]
        
        # Xóa khỏi danh sách
        del self.merger_xml_files[index]
        self.merger_file_listbox.delete(index)
        
        self.print_to_output(f"🗑️ Đã xóa file: {os.path.basename(file_path)}")
        
    def merger_clear_all(self):
        """Xóa tất cả file trong danh sách"""
        if not self.merger_xml_files:
            return
            
        self.merger_xml_files.clear()
        self.merger_file_listbox.delete(0, tk.END)
        self.print_to_output("🗑️ Đã xóa tất cả file khỏi danh sách")
        
    def merger_extract_content_elements(self, xml_content):
    ...
    def merger_merge_files(self):
        """Ghép các file XML thành một file"""
        if not self.merger_xml_files:
            self.print_to_output("⚠️ Chưa có file nào để ghép")
            return
            
        output_filename = self.merger_output_var.get().strip()
        if not output_filename:
            self.print_to_output("❌ Vui lòng nhập tên file output")
            return
            
        # Chạy trong thread để không làm treo UI
        threading.Thread(target=self._merger_merge_thread, args=(output_filename,)).start()
        
    def _merger_merge_thread(self, output_filename):
        """Thread thực hiện ghép file"""
        try:
            self.print_to_output(f"🔗 Bắt đầu ghép {len(self.merger_xml_files)} file XML...")
            ...
            for i, file_path in enumerate(self.merger_xml_files, 1):
                try:
                    self.print_to_output(f"📄 Đang xử lý file {i}/{len(self.merger_xml_files)}: {os.path.basename(file_path)}")
                    
                    with open(file_path, 'r', encoding='utf-8') as f:
                        content = f.read()
                    
                    # Trích xuất các thẻ content
                    content_elements = self.merger_extract_content_elements(content)
                    ...
                    self.print_to_output(f"   ✅ Đã thêm {len(content_elements):,} content elements")
                    
                except Exception as e:
                    self.print_to_output(f"   ❌ Lỗi xử lý file {os.path.basename(file_path)}: {str(e)}")
                    return
            ...
            self.print_to_output(f"\n🎉 Ghép file hoàn tất!")
            self.print_to_output(f"📁 File kết quả: {output_path}")
            self.print_to_output(f"📊 Tổng số content: {total_content_count:,}")
            self.print_to_output(f"📏 Kích thước file: {file_size:,} bytes ({file_size / 1024 / 1024:.2f} MB)")
            self.print_to_output(f"💡 File đã được lưu trong thư mục 'output' của project.")
                
        except Exception as e:
            self.print_to_output(f"❌ Có lỗi xảy ra khi ghép file: {str(e)}")
