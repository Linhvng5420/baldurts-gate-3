...
# Thử import tkinterdnd2 cho tính năng drag & drop
try:
    from tkinterdnd2 import DND_FILES, TkinterDnD
    DRAG_DROP_AVAILABLE = True
except ImportError:
    DRAG_DROP_AVAILABLE = False
    print("Thông báo: tkinterdnd2 không có sẵn. Tính năng kéo thả file sẽ không hoạt động.")
...
class XMLMergerApp:
    def __init__(self, root):
        ...
        # Thiết lập drag & drop nếu có sẵn
    ...
    def setup_ui(self):
        ...
        # Thiết lập drag & drop cho listbox
        if self.drag_drop_enabled:
            self.file_listbox.drop_target_register(DND_FILES)
            self.file_listbox.dnd_bind('<<Drop>>', self.on_drop)
            
            # Thêm thông báo về tính năng drag & drop
            self.drag_label = ttk.Label(list_frame, 
                                      text="💡 Bạn có thể kéo thả file XML vào danh sách để thêm nhanh!", 
                                      foreground="blue", font=("Arial", 9, "italic"))
            self.drag_label.grid(row=2, column=0, pady=(5, 0))
    ...
    def on_drop(self, event):
        ...
        # Lấy danh sách file được kéo thả
        ...
        for file_path in files:
            ...
            if not os.path.exists(file_path):
                skipped_files.append(f"{os.path.basename(file_path)} - Không tồn tại")
                continue
                
            # Kiểm tra extension
            if not file_path.lower().endswith('.xml'):
                skipped_files.append(f"{os.path.basename(file_path)} - Không phải file XML")
                continue
                
            # Kiểm tra duplicate
            if file_path in self.xml_files:
                skipped_files.append(f"{os.path.basename(file_path)} - Đã có trong danh sách")
                continue
        ...
        # Hiển thị kết quả
        if added_count > 0:
            message = f"Đã thêm {added_count} file thành công!"
            if skipped_files:
                message += f"\n\nBỏ qua {len(skipped_files)} file:\n" + "\n".join(skipped_files)
            messagebox.showinfo("Kéo thả file", message)
        elif skipped_files:
            message = f"Không thêm được file nào.\n\nLý do:\n" + "\n".join(skipped_files)
            messagebox.showwarning("Kéo thả file", message)
        
    def browse_file(self):
        """Mở dialog chọn file (có thể chọn nhiều file)"""
        filenames = filedialog.askopenfilenames(
            title="Chọn file XML (có thể chọn nhiều file)",
        ...
        if filenames:
            ...
            # Hiển thị kết quả nếu chọn nhiều file
            if len(filenames) > 1:
                message = f"Đã thêm {added_count} file thành công!"
                if skipped_files:
                    message += f"\n\nBỏ qua {len(skipped_files)} file:\n" + "\n".join(skipped_files)
                messagebox.showinfo("Thêm file", message)
            elif added_count == 1:
                # Nếu chỉ chọn 1 file thì đặt vào entry để người dùng có thể chỉnh sửa
                self.path_var.set(filenames[0])
    ...
    def merge_files(self):
        ...
        try:
            ...
            project_root = os.path.dirname(script_dir)  # Lên một cấp từ src/
...
def main():
    # Sử dụng TkinterDnD root nếu có sẵn, ngược lại dùng Tk thông thường
